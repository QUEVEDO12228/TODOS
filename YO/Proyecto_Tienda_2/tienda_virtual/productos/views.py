...
class ProductoUpdateView(UpdateView):
    ...
    # se realiza la validación de permisos
    #dispatch recibe las solitudes get y post
    def dispatch(self, request, *args, **kwargs):

        if not request.user.is_staff:
            return redirect('lista_productos')
        #permite seguir con el comportamiento de la vista si es admin (puede editar)
...
def toggle_producto(request, pk):
    ...
    if not request.user.is_staff:
        #se muestra un mensaje que indica que el usuario no tiene permisos
